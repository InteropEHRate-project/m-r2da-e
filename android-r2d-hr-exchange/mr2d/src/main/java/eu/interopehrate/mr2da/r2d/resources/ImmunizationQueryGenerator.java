...
class ImmunizationQueryGenerator extends AbstractQueryGenerator {
    ...
    public IQuery<Bundle> generateQueryForSearch(Arguments args, Options opts) {
        ...
        IQuery<Bundle> q = fhirClient
                ...
                .where(Immunization.STATUS.exactly().code(Immunization.ImmunizationStatus.COMPLETED.toCode()));
        ...
        if (args.hasArgument(ArgumentName.FROM)) {
            ...
            q = q.and(Immunization.DATE.afterOrEquals().day(from));
        }
        ...
        if (opts.hasOption(OptionName.SORT)) {
            ...
            if (sort == Option.Sort.SORT_ASCENDING_DATE)
                q = q.sort().ascending(Immunization.DATE);
            else
                q = q.sort().descending(Immunization.DATE);
        }
